...
def download_videos(videos, username):
    """Download videos and save captions and like counts to local directory."""
    ...
    for video in track(videos, description="Downloading videos..."):
        try:
            ...
            caption = video["title"]
            like_count = video.get("digg_count", "N/A")  # Get like count or default to "N/A" if not available
            video_filepath = f"./tiktok/{username}/{video_id}_HaveLike_{like_count}.mp4"
            caption_filepath = f"./tiktok/{username}/{video_id}_HaveLike_{like_count}.txt"
            ...
            # Save caption -> video_id.txt file
            ...
            time.sleep(0.7) # Sleep to avoid hitting rate limits
        except Exception as e:
