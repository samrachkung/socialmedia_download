...
class InstagramPhotoDownloader:
    ...
    def extract_caption_from_json(self, json_path):
        ...
        try:
            with open(json_path, 'r', encoding='utf-8') as f:
                ...
                # Optional: Clean up caption (remove line breaks, etc.)
    ...
    def run_gallery_dl(self, url, folder):
        """Run gallery-dl command to download Instagram content with captions"""
        try:
            # Construct gallery-dl command with Instagram-specific options
            command = [
                "gallery-dl.exe",
                "--destination", folder,
                "--write-metadata",  # Save JSON with captions
                "--filter", "extension in ['jpg', 'jpeg', 'png', 'gif', 'webp']",  # Images only
                url
            ]
            ...
            while True:
                ...
                if output:
                    ...
                    if "extension not in" in output_text.lower() or ".mp4" in output_text.lower():
            ...
            if self.process.returncode != 0 and not STOP:
                ...
                # Process captions after successful download
                if not STOP:
                    ...
                    for json_file in json_files:
                        ...
                        if caption:
                            image_base = os.path.splitext(json_file)[0]
                            image_path = os.path.join(folder, image_base + ".jpg")  # Adjust extension if needed
                            self.save_caption_as_txt(image_path, caption)
                            self.append_to_console(f"✅ Saved caption: {image_base}.txt")
