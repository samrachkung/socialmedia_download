    ...
    def process_download_queue(self):
        while True:
            try:
                ...
                if self.current_downloads == self.progress_bar['maximum']:
                    self.is_downloading = False
                    self.download_button.config(state=tk.NORMAL)
                    self.stop_button.config(state=tk.DISABLED)
                    ...
                    webbrowser.open(f"file://{self.download_folder.get()}")
    ...
    def download_image(self, url):
        try:
            config = {
                'extractor': {
                    'facebook': {
                        'metadata': True,  # Include caption and other metadata
                    }
                },
                'directory': self.download_folder.get(),
                'filename': '{filename}.{extension}',
            }
            
            # Run gallery-dl
            gallery_dl.run([url], config)
            
            self.log_message(f"Successfully downloaded image: {url}")
            return True
        except Exception as e:
            self.log_message(f"Error downloading image {url}: {str(e)}")
            return False
