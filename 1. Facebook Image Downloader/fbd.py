...
# Updated color scheme to match the TikTok downloader style
PRIMARY_COLOR = "#4267B2"    # Facebook blue
SECONDARY_COLOR = "#5B7BD5"  # Lighter blue for hover
ACCENT_COLOR = "#365899"     # Darker blue for emphasis
BG_COLOR = "#E9EBEE"         # Light gray/blue background (Facebook style)
TEXT_COLOR = "#1C1E21"       # Dark text color (Facebook style)
...
class FacebookPhotoDownloader:
    def __init__(self, root):
        self.root = root
        self.root.title("Facebook Photo Downloader")
    ...
    def setup_ui(self):
        ...
        title = tk.Label(
            title_logo_frame,
            text="Facebook Photo Downloader",
        ...
        # Logo
        try:
            self.logo_img = tk.PhotoImage(file="facebook_logo.png")
        ...
        url_label = tk.Label(
            main_frame,
            text="Enter Facebook Photo/Album URLs (one per line):",
            font=FONT_BOLD,
            bg=BG_COLOR,
            fg=TEXT_COLOR
        )
        ...
        # Pre-fill the URL input with "https://www.facebook.com/"
        ...
        creator_label = tk.Label(
            footer,
            text="This application created by Kungsamrach @ 2025",
            font=(FONT_FAMILY, 9),
            bg=BG_COLOR,
            fg=TEXT_COLOR
        )
    ...
    def start_download(self):
        """Start downloading photos from entered URLs"""
        urls = self.url_text.get("1.0", tk.END).strip().split("\n")
        
        if not urls or all(url.strip() == "" for url in urls):
            messagebox.showerror("Error", "Please enter at least one valid Facebook URL.")
            return
        ...
        self.append_to_console("Starting downloads...\n")
    ...
    def extract_caption_from_json(self, json_path):
        """Extract caption from JSON file and return it"""
        try:
            with open(json_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                # Try different possible caption locations in the JSON
    ...
    def save_caption_to_txt(self, txt_path, caption):
    ...
    def run_gallery_dl(self, url, folder):
        """Run gallery-dl command to download photos with captions"""
        try:
            # Construct gallery-dl command
            command = [
                "gallery-dl.exe",
                "--destination", folder,
                "--filename", "{post_id}_{filename}",  # Simplified filename format
                "--write-metadata",
                "--write-info-json",
                url
            ]
            ...
            while True:
                ...
                if output:
                    self.append_to_console(output.strip())
                    
                    # Check if this line indicates a successful download
                    if output.strip().endswith(('.jpg', '.jpeg', '.png')):
                        # Get the full path of the downloaded image
                        img_path = output.strip()
                        # Construct the JSON path
                        json_path = img_path + '.json'
                        if os.path.exists(json_path):
                            # Extract caption
                            caption = self.extract_caption_from_json(json_path)
                            if caption:
                                # Create txt file with same name as image
                                txt_path = os.path.splitext(img_path)[0] + '.txt'
                                self.save_caption_to_txt(txt_path, caption)
                                self.append_to_console(f"Saved caption for {os.path.basename(img_path)}")
